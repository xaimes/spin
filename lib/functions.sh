...
install_spin_package_to_project() {
  ...
  case "$framework" in
    "php")
      docker run --rm -v $project_dir:/var/www/html --user "${SPIN_USER_ID}:${SPIN_GROUP_ID}" -e "LOG_LEVEL=off" $SPIN_PHP_IMAGE composer --working-dir=/var/www/html/ require serversideup/spin:^2.0@beta --dev
      ;;
    "node")
